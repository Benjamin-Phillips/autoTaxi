...
namespace autoTaxi {
    class Program {
        ...
        //        // Console.ReadKey();
        //        // Console.Write("\b");
        //        greedyUpdate(elapsedTime, cars, requests[i].time);
        //    }
        //    Console.WriteLine();
        ...
        static public void greedyUpdate(int elapsedTime, List<Car> cars, int curTime) {
            foreach (Car car in cars) {
                ...
                while (car.requests.Count > 0 && Dispatcher.distance(curPoint, car.pos) < travelDistance) {
                    ...
                    if (car.requests[0].needsPickedUp) {
                        ...
                        car.delivered.Add(new DeliveredPassenger(car.requests[0], curTime));
        ...
        static public void update(int elapsedTime, List<Car> cars, int curTime) {
            foreach(Car car in cars) {
                double travelDistance = Car.speed * elapsedTime;
                //move car through dropoff points
                while (car.requests.Count > 0 && Dispatcher.distance(car.requests[0].end, car.pos) < travelDistance) {
                    ...
                    if(car.requests[0].passengers > 0) {
                        car.delivered.Add(new DeliveredPassenger(car.requests[0], curTime));
                    }
                    car.Passengers -= car.requests[0].passengers;
                    car.requests.RemoveAt(0);
